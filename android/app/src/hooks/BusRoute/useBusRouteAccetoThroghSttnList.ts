...
export const useBusRouteAccetoThroghSttnList = () => {
  const [routes, setRoutes] = useState<IBusViaRoute[]>([]);
  ...
  const search = useCallback(async (cityCode: number, routeId: string) => {
    ...
    try {
      const data = await getBusRouteAccetoThroghSttnList(cityCode, routeId);
