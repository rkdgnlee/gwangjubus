...
export const searchHistoryStorage = {
  ...
  async saveHistory(mode: 'bus' | 'stop', keyword: string) {
    ...
    try {
      ...
      await Storage.setItem(`${STORAGE_KEY_PREFIX}${mode}`, JSON.stringify(newHistory));
      return newHistory;
    } catch (e) {
  ...
  async getHistory(mode: 'bus' | 'stop'): Promise<string[]> {
    try {
      const jsonValue = await Storage.getItem(`${STORAGE_KEY_PREFIX}${mode}`);
  ...
  async clearHistory(mode: 'bus' | 'stop') {
    try {
      await Storage.removeItem(`${STORAGE_KEY_PREFIX}${mode}`);
    } catch (e) {
